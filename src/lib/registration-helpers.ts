...
export const validateRegistrationStep = (
  ...
  if (step === 0 && !form.accountType) e.push("Please select an account type");
    ...
    if (form.otp.length < 4)
      e.push("Enter the 4-digit code sent to your email");
    ...
    if (step === 3) {
      ...
      if (!form.nin.trim() || form.nin.length < 11)
        e.push("Valid 11-digit NIN is required");
  ...
  if (form.accountType === "driver" && step === 10 && !form.termsAccepted) {
    e.push(
      "You must accept the terms and conditions before submitting your application",
    );
  }
