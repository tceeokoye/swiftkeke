...
export default function AppDownload() {
  return (
    <section id="app" className="py-24 lg:py-32 relative overflow-hidden bg-[#F7F7F7]">
      {/* Background accent */}
      <div className="absolute left-0 top-1/4 w-80 h-80 bg-[#DE2910]/8 rounded-full blur-3xl pointer-events-none" />
          ...
          <div>
            ...
            <ul className="space-y-3 mb-10">
              {perks.map((perk) => (
                <li key={perk} className="flex items-center gap-3 text-sm text-[#555555]">
                  <div className="w-5 h-5 rounded-full bg-[#DE2910]/15 border border-[#DE2910]/30 flex items-center justify-center shrink-0">
                    <Check className="w-3 h-3 text-[#DE2910]" />
                  </div>
                  {perk}
                </li>
              ))}
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              <a
                ...
                id="appStoreBtn"
                className="flex items-center gap-4 px-6 py-4 bg-[#1A1A1A] border border-[#1A1A1A] rounded-2xl hover:bg-[#DE2910] hover:border-[#DE2910] transition-all duration-300 group"
              >
