...
export default function StepVehiclPhotos({ form, set }: Props) {
  ...
  return (
    <div className="space-y-5 animate-fadeInUp">
      <h3 className="text-xl font-bold text-[#1A1A1A] flex items-center gap-2">
        <Car className="w-5 h-5 text-[#DE2910]" /> Vehicle Photos
      </h3>
      ...
      <div className="pt-4 border-t border-gray-100 ">
        ...
        <div className="grid sm:grid-cols-3 gap-4">
          <FileUploadBox
            label="Front Photo"
            hint="Include headlights"
            file={form.frontPhoto}
            onFile={(f) => set("frontPhoto", f)}
            accept=".jpg,.jpeg,.png"
            icon={Car}
          />
          <FileUploadBox
            label="Back Photo"
            hint="Include plate number"
            file={form.backPhoto}
            onFile={(f) => set("backPhoto", f)}
            accept=".jpg,.jpeg,.png"
            icon={Car}
          />
          <FileUploadBox
            label="Side Photo"
            hint="Full side profile"
            file={form.sidePhoto}
            onFile={(f) => set("sidePhoto", f)}
            accept=".jpg,.jpeg,.png"
            icon={Car}
          />
        </div>
