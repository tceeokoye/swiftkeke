...
import {
  ChevronRight, ChevronLeft, Check, AlertCircle, Loader2, Zap, UserPlus, Lock, ShieldCheck, User, Phone, Car, MapPin, FileText, Camera
} from "lucide-react";
...
// Helpers
import { 
  initialForm, validateRegistrationStep, 
  checkPasswordStrength, getStrengthColor, getStrengthText 
...
const DRIVER_STEPS = [
  { label: "Role", icon: UserPlus }, { label: "Account", icon: Lock },
  { label: "Verify", icon: ShieldCheck }, { label: "Personal", icon: User },
  { label: "Contact", icon: Phone }, { label: "Vehicle", icon: Car },
  { label: "Address", icon: MapPin }, { label: "Documents", icon: FileText },
  { label: "Selfie", icon: Camera }, { label: "Review", icon: Check },
];

const PASSENGER_STEPS = [
  { label: "Role", icon: UserPlus }, { label: "Account", icon: Lock },
  { label: "Verify", icon: ShieldCheck }, { label: "Waitlist", icon: Phone },
...
export default function RegistrationForm() {
  ...
  const set = (key: keyof RegistrationFormData, val: any) => setForm((p) => ({ ...p, [key]: val }));

  const resendCode = (e?: React.MouseEvent) => {
    e?.preventDefault();
    sendHttpRequest({
      successRes: () => toast.success("Verification code resent"),
      requestConfig: {
        url: "/auth/resend-otp",
        method: "POST",
        body: { 
  ...
  const next = () => {
    ...
    if (step === 1) {
      sendHttpRequest({
        successRes: (res) => {
          ...
          if (data?.message === "complete_registration" || data?.data?.user?.status === "verified") {
        ...
        errorRes: (err) => {
          ...
          if (data?.description === "Email already exists!" && data?.data === null) {
        ...
        requestConfig: {
          ...
          body: { 
            ...
            confirmPassword: form.confirmPassword, userType: form.accountType 
          },
          userType: form.accountType as "driver" | "passenger",
        }
      });
    ...
    if (step === 2) {
      sendHttpRequest({
        ...
        requestConfig: {
          ...
          method: "POST",
          body: { email: form.email, otp: form.otp, userType: form.accountType },
          userType: form.accountType as "driver" | "passenger",
        }
      });
      return;
    ...
    const maxStep = form.accountType === "driver" ? 9 : (form.accountType === "passenger" ? 4 : 1);
  ...
  const handleSubmit = () => {
    ...
    if (form.accountType === "driver") {
      ...
      if (form.driversLicense) formData.append("driverLicenseFile", form.driversLicense);
      ...
      sendHttpRequest({
        ...
        errorRes: (err) => {
          const data = err.response?.data;
          if (err.response?.status === 401 || data?.description === "Not authenticated") {
  ...
  if (submitted) {
    return <RegistrationSuccess firstName={form.firstName} email={form.email} accountType={form.accountType} onReset={() => { setSubmitted(false); setForm(initialForm); setStep(0); }} />;
  }

  const currentSteps = form.accountType === "driver" ? DRIVER_STEPS : (form.accountType === "passenger" ? PASSENGER_STEPS : [{ label: "Role", icon: UserPlus }, { label: "Verification", icon: ShieldCheck }]);
            ...
            {currentSteps.map(({ label, icon: Icon }, i) => (
                <div key={label} className="flex items-center gap-0 flex-1 last:flex-initial">
                    <div className="flex flex-col items-center gap-1.5">
                        <div className={`w-10 h-10 rounded-xl flex items-center justify-center text-sm font-bold transition-all duration-300 ${i < step ? "bg-green-500/20 border border-green-500/40 text-green-600" : i === step ? "bg-[#D21F3C] text-white shadow-lg shadow-[#D21F3C]/30" : "bg-gray-100 border border-gray-200 text-[#888888]"}`}>
                            {i < step ? <Check className="w-4 h-4" /> : <Icon className="w-4 h-4" />}
                        </div>
                        <span className={`text-[10px] font-medium hidden sm:block ${i <= step ? "text-[#D21F3C]" : "text-[#888888]"}`}>{label}</span>
                    </div>
                    {i < currentSteps.length - 1 && <div className={`flex-1 h-px mx-2 transition-colors duration-300 ${i < step ? "bg-green-400" : "bg-gray-200"}`} />}
                </div>
            ))}
        </div>
        ...
        <div className="bg-white border border-gray-100 rounded-3xl p-6 sm:p-8 shadow-xl">
          {errors.length > 0 && (
            <div className="mb-6 p-4 bg-red-500/10 border border-red-500/30 rounded-xl">
              {errors.map((e, i) => <div key={i} className="flex items-center gap-2 text-sm text-red-400"><AlertCircle className="w-3.5 h-3.5 shrink-0" /> {e}</div>)}
            </div>
          )}

          {step === 0 && <StepRoleSelection accountType={form.accountType} set={set} />}
          {step === 1 && <StepAccountSetup form={form} set={set} showPassword={showPassword} setShowPassword={setShowPassword} checkPasswordStrength={checkPasswordStrength} getStrengthText={getStrengthText} getStrengthColor={getStrengthColor} />}
          {step === 2 && <StepVerifyOtp form={form} set={set} loading={loading} resendCode={resendCode} />}
          {form.accountType === "driver" && (
            <>
              ...
              {step === 8 && <StepDocuments form={form} set={set} isSelfieStep />}
              {step === 9 && <StepReview form={form} />}
            </>
          )}
          {form.accountType === "passenger" && (
            <>
              {step === 3 && <StepPersonalInfo form={form} set={set} isPassenger />}
              {step === 4 && <StepReview form={form} isPassenger />}
            </>
          )}

          <div className="flex items-center justify-between mt-8 pt-6 border-t border-gray-100">
            {step > 0 && <button onClick={() => setStep(s => s - 1)} className="flex items-center gap-2 px-5 py-3 text-sm font-semibold text-[#555555] bg-gray-100 border border-gray-200 rounded-xl transition-all cursor-pointer"><ChevronLeft className="w-4 h-4" /> Back</button>}
            <div className={step === 0 ? "flex-1" : ""} />
            {((form.accountType === "driver" && step < 9) || (form.accountType === "passenger" && step < 4) || step <= 2) ? (
              <button onClick={next} disabled={loading || (step === 0 && !form.accountType)} className="flex items-center gap-2 px-6 py-3 text-sm font-bold text-white bg-[#D21F3C] rounded-xl shadow-lg shadow-[#D21F3C]/25 hover:bg-[#a8172d] transition-all disabled:opacity-60 cursor-pointer">
                {loading ? <Loader2 className="w-4 h-4 animate-spin" /> : (step === 1 ? "Create Account" : step === 2 ? "Verify OTP" : "Continue")} 
                {!loading && <ChevronRight className="w-4 h-4" />}
              </button>
            ) : (
              <button onClick={handleSubmit} disabled={loading} className="flex items-center gap-2 px-8 py-3 text-sm font-bold text-white bg-green-600 rounded-xl shadow-lg shadow-green-700/30 hover:bg-green-700 transition-all disabled:opacity-60 cursor-pointer">
                {loading ? <><Loader2 className="w-4 h-4 animate-spin" /> Processing...</> : <><Zap className="w-4 h-4" /> {form.accountType === "passenger" ? "Join Waitlist" : "Continue"}</>}
              </button>
            )}
          </div>
        </div>
